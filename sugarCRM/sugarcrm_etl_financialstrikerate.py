...
def load_financialstrikerate_data():
 try:
   if sugarcrm_generic_module.vLoadType=='Full':
     #Full Financial Strikerate data load block
     print('*****DISABLED******************************Deleting Financial Actual data from SugarCRM')
     #sugarcrm_generic_module.delete_module_data_using_bulk_api(sugarcrm_generic_module.access_token,"COV_FinancialSummary")
     #sys.exit()
     print('Financial Strikerate data fetch for '+sugarcrm_generic_module.vCountryCode+' from EDW started @'+str(datetime.datetime.strftime(datetime.datetime.now(),'%Y-%m-%d %H:%M:%S')))
     sugarcrm_generic_module.cursor.execute("exec dbo.rptsp_rpt0998 '"+sugarcrm_generic_module.vCountryCode+"','"+sugarcrm_generic_module.vLoadType+"','"+sugarcrm_generic_module.vDateRange+"','"+sugarcrm_generic_module.vStartDate+"','"+sugarcrm_generic_module.vEndDate+"','"+sugarcrm_generic_module.vUniqueIdentifier+"'");
     print('Financial Strikerate data fetch for '+sugarcrm_generic_module.vCountryCode+' from EDW completed @'+str(datetime.datetime.strftime(datetime.datetime.now(),'%Y-%m-%d %H:%M:%S')))
     print(sugarcrm_generic_module.vCountryCode+' Financial Strikerate data push to SugarCRM started @:'+str(datetime.datetime.strftime(datetime.datetime.now(),'%Y-%m-%d %H:%M:%S')))
     jsonFARequestList=[]
     total_count=0
     loop_counter=0
     for row in sugarcrm_generic_module.cursor:
     ...
     #Fiancial Strikerate delta/differential load.
       print('Financial Strikerate data fetch for '+sugarcrm_generic_module.vCountryCode+' from EDW started @'+str(datetime.datetime.strftime(datetime.datetime.now(),'%Y-%m-%d %H:%M:%S')))
       sugarcrm_generic_module.cursor.execute("exec dbo.rptsp_rpt0998 '"+sugarcrm_generic_module.vCountryCode+"','"+sugarcrm_generic_module.vLoadType+"','"+sugarcrm_generic_module.vDateRange+"','"+sugarcrm_generic_module.vStartDate+"','"+sugarcrm_generic_module.vEndDate+"','"+sugarcrm_generic_module.vUniqueIdentifier+"'");
       #cursor.execute("exec dbo.rptsp_rpt0994 '"+vcountry_code+"',null,'Full'");
       #cursor.execute("exec dbo.rptsp_rpt0994 '"+'AU'+"','AU.CMA.AAN2400','Test'");
       #cursor.execute("exec dbo.etlsp_ETL099_scrmAccount '"+'AU'+"','AU.CMA.CTN0148'");
       print('Financial Actual Strikerate fetch for '+sugarcrm_generic_module.vCountryCode+' from EDW completed @'+str(datetime.datetime.strftime(datetime.datetime.now(),'%Y-%m-%d %H:%M:%S')))
       print(sugarcrm_generic_module.vCountryCode+' Financial Actual Strikerate push to SugarCRM started @:'+str(datetime.datetime.strftime(datetime.datetime.now(),'%Y-%m-%d %H:%M:%S')))
       loop_counter=0
       
       for row in sugarcrm_generic_module.cursor:
          value1 = row.UniqueIdentifier
          len_v = len(value1)
          vfinacial_strikerate_integration_id_c=value1[len_v-7:]
          vMonth=(datetime.datetime.strptime(row.Month,'%Y-%m-%d')).strftime('%h')
          vYear=(datetime.datetime.strptime(row.Month,'%Y-%m-%d')).strftime('%Y')
          vName=str(vfinacial_strikerate_integration_id_c)+"/"+str(vMonth)+"/"+str(vYear)
          print(vName)
          jsonFSrecord={#"account_integration_id_c":row.UniqueIdentifier,
                            ...
                            "strike_rate_c":str(float(row.StrikeRate))
                            #"sales_target_c":row.SalesTarget,
                            #"policy_count_target_c":row.PolicyCount,
                            #"currency_code_target_c":row.CurrencyCode
                            #"commission_target_c":row.,
          ...
          if len(response.json()["records"])==0:
            print('Financial Actual doesnt exist')
            #url = sugarcrm_generic_module.rest_url+"COV_FinancialSummary"
            #sugarcrm_generic_module.access_token=sugarcrm_generic_module.validate_token(sugarcrm_generic_module.access_token)
            #headers = {
            #  'OAuth-Token': sugarcrm_generic_module.access_token,
            ...
            #sugarcrm_generic_module.update_cursor.execute("update [db-au-workspace].[dbo].[scrmFinancialSummary_UAT] set isSynced='Y',SyncedDateTime=getdate() where uniqueIdentifier='"+vfinacial_actual_integration_id_c.replace("'","''")+"'")
            #sugarcrm_generic_module.update_cursor.commit()
            ...
            print('Multiple financial actual records found for vfinacial_strikerate_integration_id_c:'+vfinacial_strikerate_integration_id_c+' Raise error')
          elif len(response.json()["records"])==1:
            print('Financial strikerate exists. Updating.')
            sugarUniqId=response.json()["records"][0]["id"]
            url = sugarcrm_generic_module.rest_url+"COV_FinancialSummary/"+sugarUniqId
            sugarcrm_generic_module.access_token=sugarcrm_generic_module.validate_token(sugarcrm_generic_module.access_token)
            headers = {
            ...
            payload=json.dumps(jsonFSrecord)
            response = requests.request("PUT", url, data=payload, headers=headers)
            sugarcrm_generic_module.checkApiCallSuccess(response,'Financial strikerate update failed for payload:'+payload)
            sugarcrm_generic_module.update_cursor.execute("update [db-au-ods].[dbo].[scrmStrikeRate] set isSynced='Y',SyncedDateTime=getdate() where uniqueIdentifier='"+row.UniqueIdentifier.replace("'","''")+"'")
            print(row.UniqueIdentifier.replace("'","''"))
            sugarcrm_generic_module.update_cursor.commit()
            print('Financial strikerate updated successfully. vfinacial_strikerate_integration_id_c:'+vfinacial_strikerate_integration_id_c)
          else:
            raise;
          loop_counter=loop_counter+1
          #if loop_counter > 3: #for testing debugging only
