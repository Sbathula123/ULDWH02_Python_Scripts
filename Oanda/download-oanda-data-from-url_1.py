...
Log_fp = r"E:\ETL\Data\Oanda_files\Log"+create_time+".txt"
if os.path.isfile(Log_fp):
    os.remove(Log_fp)

url_fp = r"E:\ETL\Data\Oanda_files\Oanda.text"
Output_folder = r"E:\ETL\Data\Oanda_files"
...
if len(df) != 200:
    ...
    for i in df.index:
            ...
            if os.path.exists(Log_fp) and os.path.getsize(Log_fp) !=0: 
                print(f"[{count-1}] Json files generated successfully but there is an Error in the line [{count}] URL, full process is unsuccessful. Please refer to log for the error: "+ Log_fp)
                sys.exit(1)
            elif response.status_code == 200:
                with open(Output_folder+"\Oanda"+str(i)+".json","wb") as f:
                    f.write(data)
