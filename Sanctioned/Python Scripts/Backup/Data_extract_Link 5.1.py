...
# **Last code updated date**: 24/11/2021
...
import pandas as pd
import numpy as np
df = pd.read_excel(r'E:\ETL\Sanctioned\Source Files\designated-entities.xlsx','Individuals')
...
k[['ID','FullLName','FullFName_1','DOBString','COB','Address']] = df[['ID','LastName', 'Name_1', 'DOB_1', 'POB_1_Country', 'Address']]
...
s = s.join(p.set_index('ID')[['NameFragment']], on='ID')


# In[ ]:


s = s.join(p.set_index('ID')[['LastName']], on='ID')
...
#replace double quotes from COB
...
entNames.to_csv(r'E:\ETL\Sanctioned\Converted Files\entNames_Link5_1.csv', index=False)
...
entDOB.to_csv(r'E:\ETL\Sanctioned\Converted Files\entDOB_Link5_1.csv', index=False)
...
entSanctioned.to_csv(r'E:\ETL\Sanctioned\Converted Files\entSanctioned_Link5_1.csv', index=False)
