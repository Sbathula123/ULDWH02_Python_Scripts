...
claims_tag_site_url='https://api.meaningcloud.com/class-1.1?key=176e9b618ade0fd86651b0043054b7d1&of=xml&verbose=n&model=CLAIMTAGS&txt=' 
...
print('--------Claim tag ETL started@:'+str(datetime.datetime.now()))
vsql="""
...
from
    [db-au-cmdwh]..vClaimTagsFeeder f
...
where
    ...
    (
        select
            null
        from
            [db-au-cmdwh]..clmClaimTags t with(index(idx_clmClaimTags_ClaimKey))
        where
            t.ClaimKey = cl.ClaimKey and
            Classification = 'TAGS' and
            not
            (
            ...
            t.UpdateTime >= convert(date, getdate())
    )
order by
    cl.CreateDate desc
"""
...
print('--------Claim tag ETL finished@:'+str(datetime.datetime.now())+'\n')
#sys.exit()
#*********************************************************************************************
