...
#following comes from environment_settings now
...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = environment_settings.vGCPCredentialsFileDir+"Cover-More Data and Analytics-fd15ac400401.json"
...
def is_step_complete(module_name,step_no):
    try:
        directorypath=environment_settings.vLogDir+module_name+'.'+str(step_no)+'.done'
        #if generic_module_instance.exec_mode=='FULL':
...
def delete_complete_marker(module_name):
    try:
        directorypath=environment_settings.vLogDir+module_name+'.*.done'
